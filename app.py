...
# Safety initialization to prevent NameErrors if blocks skip execution
u_calc, v_calc, w_calc = None, None, None

if view_mode == "3D Space (X-Y-Z)":
    val_z = st.sidebar.number_input("z position:", value=1.0)
else:
    val_z = 0.0  # Kept at zero as a constant baseline for flat plane evaluation
...
if op == "Gradient":
    ...
    w_calc = sp.lambdify((x, y, z), sp.sympify("0") if view_mode == "2D Plane (X-Y)" else sp.sympify(eq_z), 'numpy')
...
if view_mode == "3D Space (X-Y-Z)":
    ...
    Z = np.zeros_like(X) # Dummy array to satisfy unified (x,y,z) dimensions input logic
